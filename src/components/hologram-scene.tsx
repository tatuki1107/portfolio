...
function StudioMobile({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    const targetX = reducedMotion ? -0.08 : state.pointer.y * 0.09 - 0.08;
    const targetY = reducedMotion ? -0.35 : state.pointer.x * 0.16 - 0.35;
    ...
    if (!reducedMotion) group.current.rotation.z += delta * 0.035;
  });

  return (
    <Float speed={reducedMotion ? 0 : 0.7} rotationIntensity={0.035} floatIntensity={0.18}>
      <group ref={group} rotation={[-0.08, -0.35, -0.12]}>
...
export function HologramScene({ reducedMotion, compact }: SceneProps) {
  return (
    ...
    >
      ...
      <mesh receiveShadow position={[0, -2.35, -0.4]} rotation={[-Math.PI / 2, 0, 0]}>
